...
object ClipsHolder:
  ...
  final case class GetClips(ref: ActorRef[Event]) extends Command

  sealed trait Event extends CborSerializable
  final case class AddedClip(content: String) extends Event
  final case class GotClips(content: List[String]) extends Event

  final case class State(clips: List[String] = Nil)
  ...
  private def commandHandler: (State, Command) => ReplyEffect[Event, State] = (state, command) => command match
    ...
    case GetClips(ref) => Effect.reply(ref)(GotClips(state.clips))
